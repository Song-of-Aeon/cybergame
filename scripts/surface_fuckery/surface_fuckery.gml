...
global.surfaces = {
	...
	postDraw: function(){
		...
		if(fancy){
			...
			draw_surface_stretched(global.surfaces.border,0,0,window_get_width(),window_get_height());
		}
		else{
			shader_set(z_detransparentify_performance)
		}
		...
		draw_surface_stretched(application_surface,((window_get_width()/2)-surface_get_width(application_surface)/2)+(gameOffset.x*global.scale),sly+(gameOffset.y*global.scale),gameSize.x*global.scale,gameSize.y*global.scale);
...
function surfaces_init(){
	with(global.surfaces){
		...
		if(fancy){
			log(external_call(global.dll_init,int64(window_handle())))
		}
		else{
			log(external_call(global.dll_disable,int64(window_handle())))
		}
